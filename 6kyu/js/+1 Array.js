...
const upArray = (arr) => {
  ...
  let numStr = arr.join("");
  let incremented = (BigInt(numStr) + 1n).toString();

 
  if (incremented.length < arr.length) {
    incremented = incremented.padStart(arr.length, "0");
  }

  return Array.from(incremented, Number);
};
